"""
Phase 2 — PGD Condition B: No Safe Replacements
-------------------------------------------------
Starting point: unsafe_prompt (original, no safe-replaced revision)
Token positions: same as Condition A (from adversarial_test.json)
random_init: False

Reads : data/df_B_pgd_input.parquet
Writes: results/2_pgd_B/results_B_no_sr.parquet
...
OUT_PATH = os.path.join(config.RES_PGD_B, 'results_B_no_sr.parquet')
...
df_B = pd.read_parquet(os.path.join(config.DATA_DIR, 'df_B_pgd_input.parquet'))
print(f'Loaded df_B: {len(df_B)} rows')


def run_pgd(df_input, random_init=False):
    final_df = []
    for category in CATEGORIES:
        print(f'\n[Condition B] Category: {category}')
...
df_out = run_pgd(df_B, random_init=False)
df_out.to_parquet(OUT_PATH, index=False)
print(f'\nCondition B saved: {len(df_out)} rows -> {OUT_PATH}')
